...
\author{\href{http://www.r-programming.org}{Guy Yollin}}
\date{Tutorial April 2016}
\institute[Copyright \copyright \ 2016]{}
...
\begin{document}
\normalem
\lstset{language=R,basicstyle=\scriptsize,tabsize=2}

%\frame{\titlepage}
\begin{frame}
\vb
\frametitle{\textcolor{purple}{Computational Finance and Risk Management}} \vspace{-1.0\baselineskip}
\begin{center}
\includegraphics[width=0.3\textwidth,keepaspectratio]{images/stockMarket.jpg} \\ \vspace{1.0\baselineskip}
\huge {\textbf{Getting started with quantstrat}} \\ \vspace{1.0\baselineskip}
\normalsize{\url{https://github.com/gyollin/quantstrat-tutorial.git}} \\
\vspace{2.0\baselineskip}
\large{Guy Yollin} \\ \vq
\scriptsize{www.r-programming.org}
\end{center}
\end{frame}
...
\begin{itemize}
    ...
    \begin{itemize}
      ...
      \item{Software Engineering roles}
        ...
        \item{Cybernetic Systems \& Automation, Inc.}
        \end{itemize}
      \item{Academic roles}
        \begin{itemize}
    ...
    \item{Contact Info}
    \begin{itemize}
        \item{\url{gyollin@r-programming.org}}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Preliminaries: quantitative analysis packages}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\begin{frame}
\frametitle{The zoo package}
The \texttt{zoo} package provides an infrastructure for regularly-spaced and irregularly-space time series \\ \vh
\textmd{Key functions:}
\begin{mydescription}{aggregate}
  \item [zoo] create a zoo time series object
  \item [merge] merges time series (automatically handles of time alignment)
  \item [aggregate] create coarser resolution time series with summary statistics
  \item [rollapply] calculate rolling window statistics
  \item [read.zoo] read a text file into a zoo time series object
\end{mydescription} \vh
\textmd{Authors:}
\begin{itemize}
  \item Achim Zeileis
  \item Gabor Grothendieck
\end{itemize}
\end{frame}

\begin{frame}
\frametitle{The xts package}
The \texttt{xts} package extends the zoo time series class with fine-grained time indexes, interoperability with other R time series classes, and user defined attributes \\ \vh
\textmd{Key functions:}
\begin{mydescription}{align.time}
  \item [xts] create an xts time series object
  \item [align.time] align time series to a coarser resolution
  \item [to.period] convert time series data to an OHLC series
  \item [ [.xts ] subset time series
\end{mydescription} \vh
\textmd{Authors:}
\begin{itemize}
  \item Jeffrey Ryan
...
\begin{frame}
\frametitle{The quantmod package}
The \texttt{quantmod} package for R is designed to assist the quantitative trader in the development, testing, and deployment of statistically based trading models. \\ \vh
\textmd{Key functions:}
\begin{mydescription}{chartSeries}
  \item [getSymbols] load or download price data
  \begin{itemize}
    \item Yahoo Finance / Google Finance
    \item FRED
    \item Oanda
    \item csv, RData
    \item MySQL, SQLite
  \end{itemize}
  \item [chartSeries] charting tool to create standard financial charts
\end{mydescription} \vh
\textmd{Author:}
\begin{itemize}
  \item Jeffrey Ryan
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Preliminaries: time-series objects}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
A \emph{time series} is a sequence of \emph{ordered} data points measured at specific points in time
\end{mydefinition}
\vh
\begin{mydefinition}{Time series object}
A time series object in R is a \emph{compound data structure} that includes a data matrix as well as a vector of associated time stamps
\end{mydefinition}
\vh
\begin{center}
\begin{tabular}{ l | l | l }
  \textbf{class} & \textbf{package} & \textbf{overview} \\
  \hline
  \texttt{ts} & stats & regularly spaced time series \\
  \texttt{mts} & stats & multiple regularly spaced time series \\
  \texttt{zoo} & zoo & reg/irreg and arbitrary time stamp classes \\
  \texttt{xts} & xts & an extension of the zoo class \\
\end{tabular}
\end{center}
\end{frame}

\begin{frame}
\frametitle{Components of an xts object}
An \texttt{xts} object is composed of 3 components:\\ \vh
\begin{mydescription}{Matrix}
  \item [Index] a Data class or Time-Date class used for the time-stamp of observations \vh
  \item [Matrix] the time series observations (univariate or multivariate)
  \begin{itemize}
    \item can be numeric, character, logical, etc. but must be homogeneous\vh
  \end{itemize}
  \item [Attr] hidden attributes and user attributes
  \begin{itemize}
    \item class of the index
    \item format of the index
    \item time zone
  \end{itemize}
\end{mydescription}
\end{frame}
...
\begin{frame}[fragile]
\frametitle{The \texttt{getSymbols} function}
The \texttt{getSymbols} function loads (downloads) historic price data \\ \vb
\textcolor{Blue}{Usage:} \vm
<<eval=FALSE>>=
getSymbols(Symbols = NULL, env = parent.frame(), src = "yahoo",
  auto.assign = getOption('getSymbols.auto.assign',TRUE), ...)
@
\textcolor{Blue}{Main arguments:}
\begin{mydescription}{auto.assign}
  \item [Symbols] a vector of ticker symbols
  \item [env] where to create objects. Setting \texttt{env=NULL} is equal to \texttt{auto.assign=FALSE}
  \item [src] source of data (yahoo)
  \item [auto.assign] should results be returned or loaded to \texttt{env}
  \item [...] additional parameters
\end{mydescription} \vh
\textcolor{Blue}{Return value:}
\begin{myindentpar}{0.75cm}
an object of type \texttt{return.class} depending on \texttt{env} and \texttt{auto.assign}
\end{myindentpar}
\end{frame}

\begin{frame}[fragile]
\frametitle{The \texttt{getSymbols.yahoo} function}
The \texttt{getSymbols.yahoo} function downloads historic price data from \texttt{finance.yahoo.com} \\ \vb
\textcolor{Blue}{Usage:} \vm
<<eval=FALSE>>=
...
@
\textcolor{Blue}{Main arguments:}
\begin{mydescription}{return.class}
...
\frametitle{The \texttt{getSymbols} function} \vm
...
@
\begin{itemize}
  \item By default, the symbol was \emph{auto-assigned} to the parent environment
\end{itemize}
\end{frame}
...
@
\begin{itemize}
  \item Note that the symbol is prepended to columns names of the xts object; use extractor functions to access column data (e.g. \code{Cl(GSPC)})
\end{itemize}
\end{frame}

\begin{frame}
\frametitle{\texttt{quantmod} extractor functions}
The \texttt{quantmod} package includes a number of functions to extract specific series from an \texttt{xts} object of market data: \\ \vh
\begin{center}
\begin{tabular}{ l l }
  \textbf{Function} & \textbf{Description} \\ [0.5em] \hline
  & \\ [-0.5em]
  Op(x) & Get Open\\ [0.5em]
  Hi(x) & Get High \\ [0.5em]
  Lo(x) & Get Low \\ [0.5em]
  Cl(x) & Get Close \\ [0.5em]
  Vo(x) & Get Volume \\ [0.5em]
  Ad(x) & Get Adjusted Close \\ [0.5em]
  HLC(x) & Get High, Low, and Close \\ [0.5em]
  OHLC(x) & Get Open, High, Low, and Close \\ [0.5em]
\end{tabular}
\end{center}
\end{frame}

\begin{frame}[fragile]
\frametitle{The \texttt{chartSeries} function}
The \texttt{chartSeries} function creates financial charts \\ \vh
\textcolor{Blue}{Usage:} \vm
<<eval=FALSE>>=
...
@
\begin{center}
\includegraphics[scale=0.5]{figures/GSPC1-1}
\end{center}
\end{frame}

\begin{frame}[fragile]
\frametitle{Customize a \texttt{chartSeries} plot}
...
@
\begin{itemize}
  \item subset to last 3 months
  \item totally white background
  \item no volume sub-graph (\texttt{TA=NULL})
\end{itemize}
\end{frame}

\begin{frame}
\frametitle{A \texttt{chartSeries} plot}
\begin{center}
\includegraphics[scale=0.6]{figures/GSPC0-1}
\end{center}
\end{frame}

\begin{frame}[containsverbatim]
\frametitle{Date class}
\vq
A \texttt{Date} object is stored internally as the number of days since 1970-01-01
<<size='tiny'>>=
myStr <- "7/4/2014"
class(myStr)
...
\begin{frame}[containsverbatim]
\frametitle{Date format string for \texttt{as.Date} and \texttt{format.Date}}
\begin{mydescription}{\%Y}
  \item [\%Y] Year with century
  \item [\%y] Year without century (00-99)
  \item [\%m] Month as decimal number (01-12)
  \item [\%d] Day of the month as decimal number (01-31)
\end{mydescription}
<<>>=
format(myDate,"%m/%d/%Y")
format(myDate,"%m/%d/%y")
...
@
\begin{itemize}
  \item For comprehensive list of date/time conversion specifications, see help for \texttt{strptime} function
\end{itemize}
\end{frame}

\begin{frame}
\frametitle{Time-Date formatting strings}
\begin{scriptsize}
\begin{description}
    \item[\code{\%a}]{Abbreviated weekday name in the current locale}
    \item[\code{\%A}]{Full weekday name in the current locale}
    \item[\code{\%b}]{Abbreviated month name in the current locale}
    \item[\code{\%B}]{Full month name in the current locale}
    \item[\code{\%c}]{Date and time.   Locale-specific on output}
    \item[\code{\%C}]{Century (00--99): the integer part of the year divided by 100.}
    \item[\code{\%d}]{Day of the month as decimal number (01--31).}
    \item[\code{\%D}]{Date format such as \code{\%m/\%d/\%y}: ISO C99 says it should be that exact format.}
    \item[\code{\%e}]{Day of the month as decimal number (1--31)}
    \item[\code{\%F}]{Equivalent to \%Y-\%m-\%d (the ISO 8601 date format)}
    \item[\code{\%g}]{The last two digits of the week-based year (see \code{\%V})}
    \item[\code{\%G}]{The week-based year (see \code{\%V}) as a decimal number}
...
\begin{frame}
\frametitle{Time-Date formatting strings (continued)}
\begin{scriptsize}
\begin{description}
    \item[\code{\%n}]{Newline on output, arbitrary whitespace on input}
    \item[\code{\%p}]{AM/PM indicator in the locale.  Used in conjunction with \code{\%I} and not with \code{\%H}}
    ...
    \item[\code{\%U}]{Week of the year as decimal number (00--53) using Sunday as the first day 1 of the week}
    \item[\code{\%V}]{Week of the year as decimal number (00--53) as defined in ISO 8601}
    \item[\code{\%w}]{Weekday as decimal number (0--6, Sunday is 0)}
    \item[\code{\%W}]{Week of the year as decimal number (00--53) using Monday as the first day of week}
    \item[\code{\%x}]{Date.  Locale-specific on output, \code{"\%y/\%m/\%d"} on input}
    \item[\code{\%X}]{Time.  Locale-specific on output, \code{"\%H:\%M:\%S"} on input}
    \item[\code{\%y}]{Year without century (00--99)}
    \item[\code{\%Y}]{Year with century}
    \item[\code{\%z}]{Signed offset in hours and minutes from UTC, so \code{-0800} is 8 hours behind UTC}
    \item[\code{\%Z}]{(Output only.)  Time zone abbreviation as a character string (empty if not available)}
\end{description}
\end{scriptsize}
\end{frame}

\begin{frame}[containsverbatim]
\frametitle{Date-Time classes}
\begin{itemize}
  \item A \texttt{POSIXct} object is a Date-Time object internally stored as the number of seconds since 1970-01-01
  \item A \texttt{POSIXlt} object is a Date-Time object internally stored as 9 calendar and time components
\end{itemize}
<<size='tiny'>>=
d <- Sys.time()
class(d)
...
\begin{frame}
\frametitle{xts subsetting}
xts time series objects can be easily subset: \\ \vh
\begin{itemize}
  \item Date and time organized from most significant to least significant
  \begin{itemize}
    \item CCYY-MM-DD HH:MM:SS[.s]
  \end{itemize} \vh
  \item Separators can be omitted
  \begin{itemize}
    \item CCYYMMDDHHMMSS
  \end{itemize} \vh
  \item Intervals can be designated with the "/" or "::"
  \begin{itemize}
    \item 2010/2011
    \item 2011-04::2011-07
    \item ::Sys.time()
    \item 2000::
  \end{itemize}
\end{itemize}
...
@
\begin{center}
\includegraphics[scale=0.5]{figures/GSPC2-1}
\end{center}
\end{frame}
...
@
\begin{center}
\includegraphics[scale=0.5]{figures/GSPC3-1}
\end{center}
\end{frame}
...
@
\begin{center}
\includegraphics[scale=0.5]{figures/GSPC4-1}
\end{center}
\end{frame}
...
@
\begin{center}
\includegraphics[scale=0.5]{figures/GSPC5-1}
\end{center}
\end{frame}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Introduction to blotter and quantstrat}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\begin{itemize}
  \item Questions \vb
  \item Download presentation and code:\\
  \url{https://github.com/gyollin/quantstrat-tutorial.git} \vb
