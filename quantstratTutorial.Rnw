...
\begin{itemize}
  \item Set timezone
  \item Use \texttt{POSIXct} for index class of quote data
\end{itemize}
\end{frame}
...
\begin{itemize}
  ...
  \item The function \texttt{strategy} initializes a new strategy object
\end{itemize}
\end{frame}
...
\begin{itemize}
  ...
  \item Rule type is '\texttt{chain}' and parent is '\texttt{LongEntry}'
\end{itemize}
\end{frame}
...
@
<<>>=
stopLossPercentRange <- seq(0.01,0.05,by=0.01)
@
<<results='hide'>>=
